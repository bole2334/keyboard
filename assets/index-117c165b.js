...
`;function ChippyLoader(e){const t=e.width||defaultChippy.width,o=e.width||defaultChippy.height,n=0,[r,s]=[o+n*2,t+n*2],i=useAppSelector(getSelectedTheme);return jsx(LoaderContainer,{children:jsx(CircleContainer,{$progress:e.progress,$progressColor:getDarkenedColor(i.accent.c,.9),$containerHeight:r,$containerWidth:s,children:jsx("div",{style:{zIndex:1,width:t},children:jsx("img",{src:xhImg,style:{width:"100%",height:"100%",objectFit:"cover",position:"absolute",top:"50%",left:"50%",transform:"translate(-50%, -50%)"},alt:"描述"})})})})}const LoadingText=styled.div`
  font-size: 30px;
  color: var(--color_label-highlighted);
...
`,Container$c=styled.div`
  ...
  flex-direction: column;
  padding: 0 12px;
`,renderEncoderError=()=>jsx(ErrorMessage,{children:"Your current firmware does not support rotary encoders. Install the latest firmware for your device."}),Pane$7=()=>{const[e,t]=reactExports.useState(),[o,n]=reactExports.useState(),r=useAppSelector(getSelectedKey),s=useAppDispatch(),i=useAppSelector(getSelectedKeyDefinitions),a=useAppSelector(h=>getSelectedKeymap(h)||[]),c=useAppSelector(getSelectedLayerIndex),l=useAppSelector(getSelectedConnectedDevice),d=useAppSelector(getSelectedKeyboardAPI),u=a[r??-1],_=i[r??-1],C=!!_&&_.col!==-1&&_.row!==-1,K=(h,y)=>{if(d&&r!==null&&_&&_.ei!==void 0){const g=+_.ei;switch(h){case"ccw":{d.setEncoderValue(c,g,!1,y),n(y);break}case"cw":{d.setEncoderValue(c,g,!0,y),t(y);break}case"click":{s(updateKey(r,y));break}}}},p=async(h,y,g)=>{const m=await g.getEncoderValue(h,y,!0),S=await g.getEncoderValue(h,y,!1);t(m),n(S)};return reactExports.useEffect(()=>{if(l&&l.protocol>=10&&_!==void 0&&_.ei!==void 0&&d){const h=+_.ei;p(c,h,d)}},[_,l,c]),_===void 0||l&&l.protocol<10||o===void 0||e===void 0?jsx(SpanOverflowCell,{children:renderEncoderError()}):jsx(SpanOverflowCell,{children:jsx(Encoder,{children:jsxs(Container$c,{children:[jsxs(ControlRow,{children:[jsx(Label$1,{children:"Rotate Counterclockwise"}),jsx(Detail,{children:jsx(PelpiKeycodeInput,{value:o,meta:{},setValue:h=>K("ccw",h)})})]}),jsxs(ControlRow,{children:[jsx(Label$1,{children:"Rotate Clockwise"}),jsx(Detail,{children:jsx(PelpiKeycodeInput,{value:e,meta:{},setValue:h=>K("cw",h)})})]}),C&&jsxs(ControlRow,{children:[jsx(Label$1,{children:"Press Encoder"}),jsx(Detail,{children:jsx(PelpiKeycodeInput,{value:u,meta:{},setValue:h=>K("click",h)})})]})]})})})},KeycodeList=styled.div`
  display: grid;
...
`,componentJoin=(e,t)=>e.reduce((o,n,r)=>(r&&o.push({...t,key:r.toString()}),o.push(n),o),[]),KeycodeMap=getKeycodes().flatMap(e=>e.keycodes).reduce((e,t)=>({...e,[t.code]:t}),{}),optimizeKeycodeSequence=e=>pipeline(e,convertCharacterTaps,trimLastWait,mergeConsecutiveWaits,foldKeydownKeyupKeys,convertToCharacterStreams),cleanKeycodeSequence=e=>pipeline(e,mergeConsecutiveWaits),MacroRecorder=({selectedMacro:e,setUnsavedMacro:t,saveMacro:o,undoMacro:n,isDelaySupported:r})=>{const[s,i]=reactExports.useState(!0),[a,c]=reactExports.useState(!1),[l,d]=reactExports.useState(!1),[u,_]=reactExports.useState(!!document.fullscreenElement),{smartOptimizeEnabled:C,recordDelaysEnabled:K}=useAppSelector(getMacroEditorSettings),p=useDispatch(),[h,y]=useKeycodeRecorder(a,K),g=reactExports.useRef(null),m=reactExports.useCallback(async f=>{c(f),f?(await navigator.keyboard.lock(),y([]),i(!1),d(!0)):(navigator.keyboard.unlock(),C&&y(optimizeKeycodeSequence(h)),d(!1))},[h,c]),S=reactExports.useCallback(()=>{o(""),i(!0),d(!1)},[y,o]),b=reactExports.useCallback(()=>{n(),y([]),i(!0),d(!1)},[n]);reactExports.useEffect(()=>{i(!0),d(!1),y([])},[e]);const E=()=>[...s?e??[]:h],M=reactExports.useMemo(()=>{let f,A=E();return s||!l||!C?f=A:f=optimizeKeycodeSequence(A),f},[h,s,C,l,e]);reactExports.useEffect(()=>{M&&t(sequenceToExpression(M))},[M]);const T=reactExports.useCallback(()=>{s&&i(!1)},[s]),D=reactExports.useCallback(f=>{const A=E();A.splice(f,1),y(cleanKeycodeSequence(A)),T()},[M,e,h,s]),O=reactExports.useCallback((f,A)=>{const R=E();R.splice(f,1,[RawKeycodeSequenceAction.Delay,A]),y(cleanKeycodeSequence(R)),T()},[M,e,h,s]),v=reactExports.useMemo(()=>componentJoin(M.map(([f,A],R)=>{const L=getSequenceItemComponent(f);return jsx(Deletable,{index:R,deleteItem:D,disabled:a,children:RawKeycodeSequenceAction.Delay!==f?jsx(L,{children:f===RawKeycodeSequenceAction.CharacterStream?componentJoin(String(A).split(" ").map((w,N)=>jsx("span",{children:w},N)),jsx("span",{style:{fontFamily:"fantasy, cursive, monospace"},children:"␣"})):Array.isArray(A)?A.map(w=>getSequenceLabel(KeycodeMap[w])??w).join(" + "):getSequenceLabel(KeycodeMap[A])}):jsx(WaitInput,{index:R,value:Number(A),updateValue:O})},`${R}-${f}`)}),jsx(SequenceLabelSeparator,{})),[M]);reactExports.useEffect(()=>{const f=()=>{_(!!document.fullscreenElement)};return document.documentElement.addEventListener("fullscreenchange",f),()=>{m(!1),document.documentElement.removeEventListener("fullscreenchange",f)}},[_]);const I=reactExports.useCallback(()=>{document.fullscreenElement?document.exitFullscreen&&(m(!1),document.exitFullscreen()):document.documentElement.requestFullscreen()},[m]);return jsxs(Fragment,{children:[jsx(MacroSequenceContainer,{ref:g,$isModified:!s,children:v.length?v:jsx(NoMacroRecorded,{children:"No macro recorded yet..."})}),jsx("div",{style:{border:"none",maxWidth:960,width:"100%",display:"flex",justifyContent:"center",transform:"translate(-0px, -21px)"},children:jsx(MacroEditControls,{isFullscreen:u,isEmpty:!e||!e.length,optimizeRecording:C,recordDelays:K,isRecording:a,addText:()=>{},deleteMacro:S,toggleOptimizeRecording:()=>{p(setMacroEditorSettings({smartOptimizeEnabled:!C}))},toggleRecordDelays:()=>{p(setMacroEditorSettings({recordDelaysEnabled:!K}))},toggleFullscreen:I,undoChanges:b,saveChanges:()=>o(),hasUnsavedChanges:!s,recordingToggleChange:m,isDelaySupported:r})})]})},TextArea=styled.textarea`
  box-sizing: border-box;
  background: var(--bg_control);
  padding: 5px 10px;
